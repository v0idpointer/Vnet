...
std::vector<std::uint8_t> SHA512::Digest(const std::span<const std::uint8_t> data) {

    std::vector<std::uint8_t> digest((SHA512::DIGEST_SIZE / 8));
    SHA512::Digest(data, digest);

    return digest;
}

void SHA512::Digest(const std::span<const std::uint8_t> data, const std::span<std::uint8_t> digest) {

    if (digest.size() < (SHA512::DIGEST_SIZE / 8))
