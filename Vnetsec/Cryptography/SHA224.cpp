...
std::vector<std::uint8_t> SHA224::Digest(const std::span<const std::uint8_t> data) {

    std::vector<std::uint8_t> digest((SHA224::DIGEST_SIZE / 8));
    SHA224::Digest(data, digest);

    return digest;
}

void SHA224::Digest(const std::span<const std::uint8_t> data, const std::span<std::uint8_t> digest) {

    if (digest.size() < (SHA224::DIGEST_SIZE / 8))
