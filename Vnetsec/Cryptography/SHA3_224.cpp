...
std::vector<std::uint8_t> SHA3_224::Digest(const std::span<const std::uint8_t> data) {

    std::vector<std::uint8_t> digest((SHA3_224::DIGEST_SIZE / 8));
    SHA3_224::Digest(data, digest);

    return digest;
}

void SHA3_224::Digest(const std::span<const std::uint8_t> data, const std::span<std::uint8_t> digest) {

    if (digest.size() < (SHA3_224::DIGEST_SIZE / 8))
        throw std::invalid_argument("'digest': Buffer too small.");

    EVP_MD_CTX* ctx = EVP_MD_CTX_new();
    if (ctx == nullptr) throw SecurityException(ERR_get_error());

    if (EVP_DigestInit_ex(ctx, EVP_sha3_224(), nullptr) != 1) {
        EVP_MD_CTX_free(ctx);
