...
std::vector<std::uint8_t> SHA256::Digest(const std::span<const std::uint8_t> data) {

    std::vector<std::uint8_t> digest((SHA256::DIGEST_SIZE / 8));
    SHA256::Digest(data, digest);

    return digest;
}

void SHA256::Digest(const std::span<const std::uint8_t> data, const std::span<std::uint8_t> digest) {

    if (digest.size() < (SHA256::DIGEST_SIZE / 8))
