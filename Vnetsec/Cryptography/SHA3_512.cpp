...
std::vector<std::uint8_t> SHA3_512::Digest(const std::span<const std::uint8_t> data) {

    std::vector<std::uint8_t> digest((SHA3_512::DIGEST_SIZE / 8));
    SHA3_512::Digest(data, digest);

    return digest;
}

void SHA3_512::Digest(const std::span<const std::uint8_t> data, const std::span<std::uint8_t> digest) {

    if (digest.size() < (SHA3_512::DIGEST_SIZE / 8))
