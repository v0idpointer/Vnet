...
std::vector<std::uint8_t> SHA3_256::Digest(const std::span<const std::uint8_t> data) {

    std::vector<std::uint8_t> digest((SHA3_256::DIGEST_SIZE / 8));
    SHA3_256::Digest(data, digest);

    return digest;
}

void SHA3_256::Digest(const std::span<const std::uint8_t> data, const std::span<std::uint8_t> digest) {

    if (digest.size() < (SHA3_256::DIGEST_SIZE / 8))
